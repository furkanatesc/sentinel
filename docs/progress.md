...
>
> Son güncelleme: 2026-08-03 (Increment 8)
...
| **8** | **Trading Terminal** (4 bölme: watchlist + candlestick grafik + order paneli + alt sekmeler) | ✅ **Branch `feat/terminal` tamam — 147/147 test, build + whole-branch review + fix wave temiz; görsel doğrulandı; merge onayı bekliyor** |
...
**Önce:** `feat/terminal` branch'inin master'a merge'i (kullanıcı onayı bekliyor).

Sonraki artım: Increment 9 = **Backtesting (+ Event Replay)** ekranı. Parametreler (strateji, tarih
aralığı, sermaye, slippage/latency/liquidity modelleri, filtreler), sonuç metrikleri (net PnL, win
rate, profit factor, Sharpe/Sortino, max drawdown, rug exposure), grafikler (equity curve, drawdown,
monthly return, trade distribution, entry/exit noktaları) ve **Event Replay** (look-ahead bias'sız
timeline oynatma). Seam'e `getBacktest*` endpoint'leri eklenir. Kendi spec → plan → SDD döngüsünden geçer.
